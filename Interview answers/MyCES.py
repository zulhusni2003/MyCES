...
def find_pythagorean_triplet(target_sum):
    # Loop untuk find value a
    for a in range(1, target_sum):
        # Loop untuk cari nilai b, dengan syarat b >= a
        for b in range(a, target_sum - a):
            c = target_sum - a - b
            # check if triplet ialah triplet Pythagorean
            if a**2 + b**2 == c**2:
                return a, b, c
...
# Cari triplet Pythagorean
triplet = find_pythagorean_triplet(target_sum)

# Check kalau triplet jumpa x
if triplet:
    # Kira hasil darab
